...
def add_data_for_name(name_data, year, rank, name):
    ...
    # check year
    if year not in name_data[name]:  # first time
        name_data[name][year] = rank
    else:  # the name+year with multiple ranks
...
def add_file(name_data, filename):
    ...
    with open(filename, 'r') as f:
        year = f.readline().strip() # the first line + strip to remove '/n' and space
        for line in f:
            line = line.strip()
            if line != '': # have data
                tokens = line.split(',')
